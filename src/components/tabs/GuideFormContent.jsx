...
import {
  fetchTourNames, fetchGuideLockStatus,
  fetchGuideRecords, addGuideRecord, updateGuideRecord, deleteGuideRecord,
  fetchGuideExpense, addGuideExpense, updateGuideExpense, deleteGuideExpense,
  fetchGuideDailyFee, addGuideDailyFee, updateGuideDailyFee, deleteGuideDailyFee,
} from '../../api/auth';
...
export default function GuideFormContent() {
  ...
  const [dailyFeeModal, setDailyFeeModal] = useState({ mode: null, data: null });
  ...
  const load = async () => {
    try {
      const [lock, t, r, e, d] = await Promise.all([
        fetchGuideLockStatus(), fetchTourNames(),
        fetchGuideRecords(), fetchGuideExpense(), fetchGuideDailyFee(),
      ]);
  ...
  // ── 수입 모달 열기 ──
  const openIncomeAdd  = () => { setIncomeForm(emptyIncome); setIncomeModal({ mode: 'add' }); };
  const openIncomeEdit = (row) => {
    setIncomeForm({
      tourName: row.tourName, representativeName: row.representativeName,
      paymentType: row.paymentType, amount: row.amount || '', headcount: row.headcount || '',
    });
  ...
  // ── 지출 모달 열기 ──
  const openExpenseAdd  = () => { setExpenseForm(emptyExpense); setExpenseModal({ mode: 'add' }); };
  const openExpenseEdit = (row) => {
    setExpenseForm({
      expenseType: row.expenseType, amount: row.amount,
      headcount: row.headcount, paymentType: row.paymentType,
    });
  ...
  const handleSubmitExpense = async (e) => {
    e.preventDefault(); setError(''); setSuccess('');
    if (checkLocked()) return;
    if (!expenseForm.amount || !expenseForm.headcount) { setError('금액과 인원을 입력해주세요.'); return; }
    try {
      ...
      setExpenseModal({ mode: null }); load();
    } catch (err) { setError(err.response?.data?.error || '처리 실패'); }
  };
  ...
  // ── 일비 모달 열기 ──
  const openFeeAdd  = () => { setDailyFeeForm(emptyFee); setDailyFeeModal({ mode: 'add' }); };
  ...
  const handleSubmitFee = async (e) => {
    e.preventDefault(); setError(''); setSuccess('');
    if (checkLocked()) return;
    if (!dailyFeeForm.amount || !dailyFeeForm.date) { setError('금액과 날짜를 입력해주세요.'); return; }
    try {
      if (dailyFeeModal.mode === 'add') {
        await addGuideDailyFee(Number(dailyFeeForm.amount), dailyFeeForm.date);
        setSuccess('일비가 추가되었습니다.');
      } else {
        await updateGuideDailyFee(dailyFeeModal.data.id, Number(dailyFeeForm.amount), dailyFeeForm.date);
        setSuccess('일비가 수정되었습니다.');
      }
      setDailyFeeModal({ mode: null }); load();
    } catch (err) { setError(err.response?.data?.error || '처리 실패'); }
  };
  ...
  const payBadge = (type) => ({ '현금': { background: '#d1fae5', color: '#065f46' }, '카드': { background: '#dbeafe', color: '#1e40af' }, '그외': { background: '#f3f4f6', color: '#555' } }[type] || { background: '#f3f4f6', color: '#555' });
  const expTypeBadge = (type) => type === '북한관수수료' ? { background: '#fef3c7', color: '#92400e' } : { background: '#ede9fe', color: '#5b21b6' };

  const SelectBtn = ({ options, value, onChange, badgeFn }) => (
    <div style={{ display: 'flex', gap: '8px' }}>
      {options.map(opt => (
        <button key={opt} type="button" onClick={() => onChange(opt)} style={{
          flex: 1, padding: '9px', border: '1.5px solid', borderRadius: '8px', fontSize: '13px', cursor: 'pointer',
  ...
  const ActionBtns = ({ onEdit, onDelete, locked }) => (
    <div style={{ display: 'flex', gap: '4px' }}>
      {!locked && <button onClick={onEdit} style={{ padding: '4px 10px', background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>수정</button>}
      {!locked && <button onClick={onDelete} className="delete-btn">삭제</button>}
    </div>
  );
  ...
  return (
    <div className="gf-wrapper">
      <div className="gf-header">
        ...
        {!isLocked && (
          <div style={{ display: 'flex', gap: '8px' }}>
            <button className="btn-primary" onClick={openIncomeAdd}>＋ 수입 추가</button>
            <button className="btn-outline" onClick={openExpenseAdd}>＋ 지출 추가</button>
            <button className="btn-outline" onClick={openFeeAdd}>＋ 일비 추가</button>
          </div>
        )}
      </div>
      ...
      <div className="summary-grid">
        <div className="summary-card"><div className="summary-label">현금 수입합계</div><div className="summary-value cash">{fmt(cashTotal)}</div></div>
        <div className="summary-card"><div className="summary-label">지출합계 (현금)</div><div className="summary-value expense">{fmt(expCashTotal)}</div></div>
        <div className={`summary-card ${netTotal >= 0 ? 'positive' : 'negative'}`}>
          <div className="summary-label">토탈</div>
          <div className={`summary-value ${netTotal >= 0 ? 'plus' : 'minus'}`}>{netTotal >= 0 ? '+' : ''}{fmt(netTotal)}</div>
        </div>
      </div>
      ...
      {activeTab === 'records' && (
        <div className="gf-table-wrap">
          <table className="gf-table">
            <thead><tr><th>투어이름</th><th>대표자</th><th>결제</th><th>금액(1인)</th><th>인원</th><th>합계</th><th></th></tr></thead>
            <tbody>
              {records.length === 0 ? <tr><td colSpan={7} className="empty">수입 내역이 없습니다.</td></tr>
              : records.map(r => (
                <tr key={r.id}>
                  <td>{r.tourName}</td>
                  ...
                  <td><ActionBtns locked={isLocked} onEdit={() => openIncomeEdit(r)} onDelete={() => handleDeleteRecord(r.id)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {activeTab === 'expense' && (
        <div className="gf-table-wrap">
          <table className="gf-table">
            <thead><tr><th>항목</th><th>결제</th><th>금액(1인)</th><th>인원</th><th>합계</th><th></th></tr></thead>
            <tbody>
              {expenses.length === 0 ? <tr><td colSpan={6} className="empty">지출 내역이 없습니다.</td></tr>
              : expenses.map(e => (
                <tr key={e.id}>
                  ...
                  <td><ActionBtns locked={isLocked} onEdit={() => openExpenseEdit(e)} onDelete={() => handleDeleteExpense(e.id)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {totalDailyFee > 0 && (
            <div className="daily-fee-card">
              ...
              <div className="daily-fee-grid">
                ...
                <div><div className="daily-label">신고액</div><div className="daily-value tax">- {fmt(taxAmount)}</div></div>
                <div><div className="daily-label">실수령액</div><div className="daily-value actual">{fmt(actualDailyFee)}</div></div>
              </div>
            </div>
          )}
          <div className="gf-table-wrap">
            <table className="gf-table">
              ...
              <tbody>
                {dailyFees.length === 0 ? <tr><td colSpan={3} className="empty">일비 내역이 없습니다.</td></tr>
                : dailyFees.map(d => (
                  <tr key={d.id}>
                    <td>{d.date}</td>
                    <td className="td-right" style={{ fontWeight: 600, color: '#1d4ed8' }}>{fmt(d.amount)}</td>
                    <td><ActionBtns locked={isLocked} onEdit={() => openFeeEdit(d)} onDelete={() => handleDeleteFee(d.id)} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* 수입 모달 */}
      {incomeModal.mode && (
        <div className="modal-bg" onClick={() => setIncomeModal({ mode: null })}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3 className="modal-title">{incomeModal.mode === 'add' ? '수입 추가' : '수입 수정'}</h3>
            <form onSubmit={handleSubmitIncome} className="modal-form">
              <div className="field"><label>투어이름 *</label>
              ...
              <div className="field"><label>대표자이름</label>
              ...
              <div className="field"><label>결제유형 *</label>
              ...
              {(incomeForm.paymentType === '현금' || incomeForm.paymentType === '카드') && (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div className="field"><label>금액 (1인)</label>
                  ...
                  {incomeForm.amount && incomeForm.headcount && (
                    <div className="total-preview">합계: <strong>{fmt(previewIncomeTotal())}</strong></div>
                  )}
                </>
              )}
              {error && <p className="field-error">⚠ {error}</p>}
              <div className="modal-btns">
                <button type="button" className="btn-outline" onClick={() => setIncomeModal({ mode: null })}>취소</button>
                <button type="submit" className="btn-primary">{incomeModal.mode === 'add' ? '추가' : '수정'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* 지출 모달 */}
      {expenseModal.mode && (
        <div className="modal-bg" onClick={() => setExpenseModal({ mode: null })}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3 className="modal-title">{expenseModal.mode === 'add' ? '지출 추가' : '지출 수정'}</h3>
            <form onSubmit={handleSubmitExpense} className="modal-form">
              <div className="field"><label>항목 *</label>
              ...
              {expenseForm.amount && expenseForm.headcount && (
                <div className="total-preview">합계: <strong>{fmt(previewExpenseTotal())}</strong></div>
              )}
              {error && <p className="field-error">⚠ {error}</p>}
              <div className="modal-btns">
                <button type="button" className="btn-outline" onClick={() => setExpenseModal({ mode: null })}>취소</button>
                <button type="submit" className="btn-primary">{expenseModal.mode === 'add' ? '추가' : '수정'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* 일비 모달 */}
      {dailyFeeModal.mode && (
        <div className="modal-bg" onClick={() => setDailyFeeModal({ mode: null })}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3 className="modal-title">{dailyFeeModal.mode === 'add' ? '일비 추가' : '일비 수정'}</h3>
            <form onSubmit={handleSubmitFee} className="modal-form">
              ...
              <div className="field"><label>일비 금액 *</label>
              ...
              {dailyFeeForm.amount && (
                <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '8px', padding: '12px', fontSize: '13px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span>신고액 (3.3%)</span>
                    <span style={{ color: '#e53e3e', fontWeight: 600 }}>- {Math.round(Number(dailyFeeForm.amount) * TAX_RATE).toLocaleString()}원</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>실수령액</span>
                    <span style={{ color: '#059669', fontWeight: 700 }}>{(Number(dailyFeeForm.amount) - Math.round(Number(dailyFeeForm.amount) * TAX_RATE)).toLocaleString()}원</span>
                  </div>
                </div>
              )}
              {error && <p className="field-error">⚠ {error}</p>}
              <div className="modal-btns">
                <button type="button" className="btn-outline" onClick={() => setDailyFeeModal({ mode: null })}>취소</button>
                <button type="submit" className="btn-primary">{dailyFeeModal.mode === 'add' ? '추가' : '수정'}</button>
              </div>
            </form>
          </div>
        </div>
