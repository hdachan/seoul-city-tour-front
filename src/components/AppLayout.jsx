...
import { DevContent } from './tabs/OtherContents';
...
const ALL_TABS = [
  ...
  { id: 'sales-admin',  label: '📊 영업 정산관리',   roles: ['ROLE_ADMIN', 'ROLE_DEV'] },
  { id: 'guide-form',   label: '📝 가이드 정산',     roles: ['ROLE_GUIDE'] },
  { id: 'sales',        label: '💼 영업 정산',       roles: ['ROLE_SALES'] },
...
function AppLayout() {
  ...
  useEffect(() => {
    if (!tokenRole || isTokenExpired()) { clearSession(); return; }
    const tab = ALL_TABS.find(t => t.id === activeTab);
    if (tab && !tab.roles.includes(tokenRole)) setActiveTab(visibleTabs[0]?.id || '');
  }, [tokenRole, activeTab]);
  ...
  const handleTabClick = (tab) => {
    if (!tab.roles.includes(tokenRole)) { alert('접근 권한이 없습니다.'); return; }
    setActiveTab(tab.id);
  };
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'sales-admin': return <SalesAdminContent />;
    ...
    <div className="layout-wrapper">
      <nav className="layout-nav">
        ...
        <div className="nav-right">
          {tokenRole && <span className={`badge ${ROLE_STYLE[tokenRole]?.cls}`}>{ROLE_STYLE[tokenRole]?.label}</span>}
          <span className="nav-user">{name}</span>
          <button className="logout-btn" onClick={() => clearSession()}>로그아웃</button>
        </div>
      </nav>
      <div className="tab-bar">
        {visibleTabs.map(tab => (
          <button key={tab.id} className={`tab-btn ${activeTab === tab.id ? 'active' : ''}`}
            onClick={() => handleTabClick(tab)}>{tab.label}</button>
        ))}
      </div>
